...
#define STORE                                                                  \
  49 // "110001"; -- STORE END Rx -- M[END] <- Rx  Format: < inst(6) | Rx(3) |
     // xxxxxxx >  + 16bit END
#define LOADN                                                                  \
  56 // "111000"; -- LOAD Rx Nr  (b0=0)   -- Rx <- Nr    ou   Load SP Nr (b0=1)
     // -- SP <- Nr    Format: < inst(6) | Rx(3) | xxxxxxb0 >  + 16bit Numero
#define LOADI                                                                  \
...
35 // "100011"; -- DIV Rx Ry Rz 			-- Rx <- Ry / Rz / Rx <-
     ...
     // Ry(3) | Rz(3)| C >
#define INC                                                                    \
  36 // "100100"; -- INC Rx / DEC Rx                 		-- Rx <- Rx + 1
     // / Rx <- Rx - 1  -- b6= INC/DEC : 0/1	Format: < inst(6) | Rx(3) | b6 |
     // xxxxxx >
#define LMOD                                                                   \
...
#define LOR                                                                    \
  19 // "010011"; -- OR Rx Ry Rz   	-- Rz <- Rx OR Ry
     // Format: < inst(6) | Rx(3) | Ry(3) | Rz(3)| x >
#define LXOR                                                                   \
...
#define LNOT                                                                   \
  21 // "010101"; -- NOT Rx Ry       	-- Rx <- NOT(Ry)
     // Format: < inst(6) | Rx(3) | Ry(3) | xxxx >
#define SHIFT                                                                  \
  16 // "010000"; -- SHIFTL0 Rx,n / SHIFTL1 Rx,n / SHIFTR0 Rx,n / SHIFTR1 Rx,n /
     // ROTL Rx,n / ROTR Rx,n
...
#define JMP                                                                    \
  2 // "000010"; -- JMP END    -- PC <- 16bit END 							  : b9-b6 =
    // COND Format: < inst(6) | COND(4) | xxxxxx >   + 16bit END
#define CALL                                                                   \
  3 // "000011"; -- CALL END   -- M[SP] <- PC | SP-- | PC <- 16bit END   : b9-b6
    // = COND	  	Format: < inst(6) | COND(4) | xxxxxx >   + 16bit END
#define RTS                                                                    \
...
#define NOP                                                                    \
  0 // "000000"; -- NOP            -- Do Nothing	 						Format: < inst(6)
    // | xxxxxxxxxx >
#define HALT                                                                   \
...
unsigned int HD[TAMANHO_MEMORIA];     // Vetor que representa o HD
...
//  Processa dados do Arquivo HD.TXT
void le_arquivo_hd(void);

//  Salvar dados no Arquivo HD.TXT ao sair da execução
void escreve_arquivo_hd(void);
...
int main() {
  ...
  int PC = 0, IR = 0, SP = 0, MAR = 0, rx = 0, ry = 0, rz = 0, COND = 0, RW = 0,
      DATA_OUT = 0, DATA_OUT_HD = 0, RWHD = 0;
  ...
  int LoadReg[8] = {0};
  int carry =
      0; // Flag do IR que indica se a ULA vai fazer operação com carry ou não
  int opcode = 0;
  int temp = 0;
...
loop:
  ...
  case STATE_RESET:
    ...
    PC = 0; // inicializa na linha Zero da memoria -> Programa tem que comecar
            // na linha Zero !!
    IR = 0;
    MAR = 0;
  ...
  temp = 0;
  for (i = 16; i--;)
    temp = temp + (int)(FR[i] * (power(2.0, i)));
...
// Função para converter de inteiro para binário
int converteBinario(int num, int posicao){
...
void le_arquivo(void) {
  ...
  while (
      fscanf(stream, "%s", linha) !=
      EOF) // Le linha por linha ate' o final do arquivo: eof = end of file !!
  {
    char letra[2] = "00";
...
void le_arquivo_hd(void) {
  ...
  FILE *stream; // Declara ponteiro para o arquivo
  int i, j;
  int processando = 0; // Flag para varrer o arquivo hd.txt e tirar o cabecalho

  if ((stream = fopen("hd.txt", "r")) == NULL) // Abre o arquivo para leitura
  {
    
    stream = fopen("hd.txt", "w");
    
    fprintf(stream, "%s %s %s %s", "--Codigo", "gerado", "pelo", "montador\n");
    fprintf(stream, "%s", "WIDTH=16;\n");
    fprintf(stream, "%s", "DEPTH=32768;\n");
    fprintf(stream, "%s", "ADDRESS_RADIX=UNS;\n");
    fprintf(stream, "%s", "DATA_RADIX=BIN;\n");
    fprintf(stream, "%s", "CONTENT BEGIN\n");
  
    for (int i = 0; i < 32768; i++) {
      fprintf(stream, "%d:%.16u;\n", i, HD[i]);
    }
...
//  Grava dados no Arquivo HD.TXT
void escreve_arquivo_hd(void) {
  
  FILE *stream; // Declara ponteiro para o arquivo
  
  if ((stream = fopen("hd.txt", "w")) == NULL) // Abre o arquivo para leitura
  {
    printf("Error: File not OPEN!");
    exit(1);
  }

  fprintf(stream, "--Codigo gerado pelo montador\n");
  fprintf(stream, "WIDTH=16;\n");
  fprintf(stream, "DEPTH=32768;\n");
  fprintf(stream, "ADDRESS_RADIX=UNS;\n");
  fprintf(stream, "DATA_RADIX=BIN;\n");
  fprintf(stream, "CONTENT BEGIN\n");
  
  for (int i = 0; i < 32768; i++) {
    fprintf(stream, "%d:", i);
    for(int j = 0; j < 16; j++){
      fprintf(stream, "%d", converteBinario(HD[i],j));
    }
    fprintf(stream, ";\n");
  }
...
ResultadoUla ULA(unsigned int x, unsigned int y, unsigned int OP, int carry) {
  ...
  for (int i = 16; i--;)
    auxFR = auxFR + (int)(auxFRbits[i] * (power(2.0, i)));
